...
# ==================== SUPPORT REPLY (WITH EDIT SUPPORT) ====================
...
async def handle_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if db.is_user_blocked(user_id):
        if not update.edited_message:
            await msg.reply_text("⚠️ User is banned.")
        return

    try:
        if update.edited_message:
            # ✅ EDIT LOGIC: पुराने भेजे गए मैसेज को ढूंढना
            old_entry = db.messages.find_one({"admin_msg_id": msg.message_id})
            if old_entry and msg.text:
                await context.bot.edit_message_text(
                    chat_id=user_id,
                    message_id=old_entry['user_side_msg_id'],
                    text=msg.text
                )
        else:
            # ✅ SEND LOGIC: नया मैसेज भेजना और ID सेव करना
            sent_msg = await context.bot.copy_message(
                chat_id=user_id,
                from_chat_id=SUPPORT_GROUP_ID,
                message_id=msg.message_id
            )
            # मैसेज मैपिंग सेव करें ताकि बाद में एडिट हो सके
        ...
        logger.error(f"Reply/Edit error: {e}")
        if not update.edited_message:
            await msg.reply_text(f"❌ Failed to send/edit: {e}")
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f"👋 Hello {update.effective_user.first_name}! Send your message to support.")
...
def main():
    ...
    logger.info("✅ Support Bot Running with Edit Support!")
    app.run_polling()
