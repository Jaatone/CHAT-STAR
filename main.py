#!/usr/bin/env python3
"""
Support Bot - Customer Support with Forum Topics + Auto-Reply + Ban System
...
# ==================== KOYEB HEALTH CHECK SETUP ====================
class HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        self.send_response(200)
        self.send_header('Content-type', 'text/plain')
        self.end_headers()
        self.wfile.write(b"Bot is running perfectly on Koyeb!")
            ...
            self.users.update_one(
                {"user_id": user_id},
                {
                    ...
                    "$setOnInsert": {
                        ...
                        "is_banned": False  # Add default ban status
                    }
                },
                upsert=True
            )
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = str(update.effective_user.id)
    if db.is_user_banned(user_id):
        return  # Banned user starting the bot silently ignored
        
    # यूज़र का पूरा नाम और यूज़रनेम प्राप्त करना
    ...
    welcome_message = (
        f"🌟 <b>Welcome to Support, {full_name}{username_text}!</b> 🌟\n\n"
        "🇬🇧 <b>How can we help you today?</b>\n"
        "Drop your message, photo, or query below. Our team will get back to you shortly! ⚡️\n"
        "<i>(⚠️ Please do not block this bot to receive our replies)</i>\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        f"🇮🇳 <b>सपोर्ट में आपका स्वागत है, {full_name}!</b>\n"
        "अपना संदेश, फोटो या समस्या यहाँ नीचे भेजें। हमारी टीम जल्द ही आपको उत्तर देगी! ⚡️\n"
        "<i>(⚠️ कृपया उत्तर पाने के लिए इस बॉट को ब्लॉक न करें)</i>\n\n"
        "👇 <b>Type your message below / अपना संदेश नीचे लिखें:</b>"
    )
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_message = (
        "ℹ️ <b>इस बॉट का उपयोग कैसे करें:</b>\n\n"
        "1️⃣ बस अपना संदेश या प्रश्न टाइप करके भेजें।\n"
        "2️⃣ आपको तुरंत एक पुष्टिकरण (confirmation) संदेश मिलेगा ✅\n"
        "3️⃣ हमारी सपोर्ट टीम आपका संदेश देखेगी।\n"
        "4️⃣ आपको अपना उत्तर यहीं प्राप्त होगा।\n\n"
        "💬 सभी प्रकार के मीडिया और संदेश समर्थित हैं!"
    )
...
async def ban_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.message_thread_id:
        await update.message.reply_text("⚠️ कृपया इस कमांड का उपयोग यूज़र के टॉपिक के अंदर करें।")
        return
    ...
    if not user:
        await update.message.reply_text("❌ इस टॉपिक से जुड़ा कोई यूज़र नहीं मिला।", message_thread_id=topic_id)
        return
        
    user_id = user['user_id']
    if db.ban_user(user_id):
        await update.message.reply_text(f"✅ <b>यूज़र बैन कर दिया गया है!</b>\n(ID: <code>{user_id}</code>)\nअब इनके मैसेज इस टॉपिक में नहीं आएँगे।", parse_mode='HTML', message_thread_id=topic_id)
    else:
        await update.message.reply_text("❌ यूज़र को बैन करने में एरर आया।", message_thread_id=topic_id)
...
async def unban_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if db.unban_user(user_id):
        await update.message.reply_text(f"✅ <b>यूज़र को अनबैन (Unban) कर दिया गया है!</b>\n(ID: <code>{user_id}</code>)\nअब वे दोबारा मैसेज भेज सकते हैं।", parse_mode='HTML', message_thread_id=topic_id)
    else:
        await update.message.reply_text("❌ यूज़र को अनबैन करने में एरर आया।", message_thread_id=topic_id)
...
def main():
    ...
    # Start the Koyeb Health Check Server in a background thread
    health_thread = threading.Thread(target=run_health_check, daemon=True)
    health_thread.start()
    logger.info(f"✅ Koyeb Health Check Server running on Port {os.environ.get('PORT', 8000)}")
    logger.info("=" * 60)
