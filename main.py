...
async def handle_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Don't reply to bot's own welcome messages
    ...
    logger.info(f"Attempting to reply to user {user_id} in topic {topic_id}")
    ...
    try:
        if update.edited_message:
            ...
            if entry and msg.text:
                ...
                logger.info(f"Edited message for user {user_id}")
        else:
            # Send Logic
            if msg.text:
                sent_msg = await context.bot.send_message(chat_id=user_id, text=msg.text)
            else:
        ...
        await msg.reply_text(f"❌ <b>FAILED:</b> User has blocked the bot.")
    except Exception as e:
        logger.error(f"Reply Error: {e}")
        await msg.reply_text(f"❌ <b>ERROR:</b> {e}")
